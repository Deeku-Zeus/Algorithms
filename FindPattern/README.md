...
3. **Start comparing** the pattern with the text from the beginning. 
4. If there’s a match, move both the pattern and text pointers forward. 5. **When a mismatch occurs**:
...
5. If a full match is found, return the starting index.
6. If the end of the text is reached without finding a match, return `-1`.
...
4. For each new window, **recalculate the hash**:
...
      
**Step 2: Pattern Matching** 
2. **Start comparing** the pattern with the text from the rightmost character of the current window.
